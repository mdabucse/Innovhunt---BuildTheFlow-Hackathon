...
export default ({ label, value, className = '', validate, setAllWeeks = () => {}, ...props }: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const startWeek = await productsService.getWeekNumber(startDate, 0);
      const result = await productsService.getProductsCountByWeek(startWeek , startWeek + 32, startDate.getFullYear());
      setWeeks(result);
      setAllWeeks(result);
